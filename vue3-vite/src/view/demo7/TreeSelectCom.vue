<template>
    <div>
        <el-popover placement="bottom" title="Title" :width="200" trigger="click" style="z-index: 9999 !important" @show="showPopover">
            <template #reference>
                <ElSelect slot="reference" ref="select" filterable :filter-method="filterable" :size="'small'" v-model="returnDataKeys" :multiple="multiple" :clearable="clearable" :collapse-tags="collapseTags" @click.native="selectClick" @remove-tag="removeTag" @clear="clean" :teleported="false" class="tree-select">
                    <ElOption v-for="item in options" :key="item.value" :label="item.label" :value="item.value"> </ElOption>
                </ElSelect>
            </template>
            <template #default>
                <ElTree class="common-tree" :width="width" ref="tree" :data="data" :props="obj" :show-checkbox="multiple" :node-key="obj.id" :check-strictly="checkStrictly" :default-expanded-keys="defaultKeys" :expand-on-click-node="multiple && expandClickNode" :check-on-click-node="checkClickNode" :highlight-current="true" @check-change="nodeClick" @node-click="nodeClick"></ElTree>
            </template>
        </el-popover>
    </div>
</template>
    ...
    setup(props: any, { emit }: any) {
            ...
            if (props.defaultKey != undefined && props.defaultKey.length > 0) {
                if (props.multiple) {
                    ...
                    if (Object.prototype.toString.call(props.defaultKey).indexOf("Array") != -1) {
                        if (Object.prototype.toString.call(props.defaultKey[0]).indexOf("Object") != -1) {
                            //对象
                            setDatas(props.defaultKey)
                        } else if (Object.prototype.toString.call(props.defaultKey[0]).indexOf("Number") != -1 || Object.prototype.toString.call(props.defaultKey[0]).indexOf("String") != -1) {
                    ...
                    // 单选
                    if (Object.prototype.toString.call(props.defaultKey).indexOf("Number") != -1 || Object.prototype.toString.call(props.defaultKey).indexOf("String") != -1 || Object.prototype.toString.call(props.defaultKey).indexOf("Object") != -1) {
        ...
        const nodeClick = (data?: any, node?: Str) => {
            if (!props.multiple) {
                ...
                var t: any[] = []
                dataInfo.options = checkedKeys.map((item: any) => {
                    //设置option选项
