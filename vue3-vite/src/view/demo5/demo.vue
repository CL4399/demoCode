<template>
    ...
    <TreeSelect v-model:value="value" v-model:searchValue="searchValue" show-search tree-default-expand-all style="width: 100%" :tree-data="treeData" tree-checkable allow-clear :show-checked-strategy="SHOW_PARENT" placeholder="Please select" :filterTreeNode="filterTreeOption">
        <template #title="{ value: val, title }">
            ...
            一
            <template v-for="(fragment, i) in val.toString().split(new RegExp(`(?<=${searchValue})|(?=${searchValue})`, 'i'))">
                <span v-if="fragment.toLowerCase() === searchValue.toLowerCase()" :key="i" style="color: aqua">
                    {{ fragment }}
                </span>
                <template style="color: aqua" v-else>{{ fragment }}</template>
    ...
    <div>
        ...
        <Tree :expanded-keys="expandedKeys" checkable :auto-expand-parent="autoExpandParent" :tree-data="gData" @expand="onExpand" @check="check">
...
export default defineComponent({
    components: { ModalCom, Modal, HeartFilled, Input, TreeSelect, Tree, InputSearch },
