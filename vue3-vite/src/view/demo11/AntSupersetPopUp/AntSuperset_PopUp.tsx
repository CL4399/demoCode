...
const AntSuperset_PopUp = (props: POPUPPROPS) => {
  ...
  const [state, setState] = useState<STATE>({
    ...
    top: `calc(50% - ${props.height ? props.height : '60%'} / 2)`,
    left: `calc(50% - ${props.width ? props.width : '50%'} / 2)`,
    transition: '.2s',
    transform: '0',
  });
  ...
  const deletePopu = () => {//删除
    try {
      ...
      IDS.style.animation = ` ${props.animatioendClass ? props.animatioendClass : 'AntSuperset_PopUp_Dongs'} ${props.animatioNTime ? props.animatioNTime : .5}s`;
  ...
  useEffect(() => {
    try {
      ...
      Div.style.animation = `${props.animatioStartClass ? props.animatioStartClass : 'AntSuperset_PopUp_Dongsd'} ${time ? time : .5}s`;
    } catch (e) {
...
const PopUp = (res: POPUPPROPS) => { //追加元素
  ...
  // div.name=res.animatioNTime
  // div.data=res.animatioStartClass
  // div.data=res.animatioendClass
...
const PopUpDelete = (id: string) => {
  if (document.getElementById(`${id}AntSuperset_PopUpHtml`)) {
    try {
      ...
      IDS.className += ' AntSuperset_PopUp_Dong';
      IDS.style.animation = ` ${Div.dataset.animatioendClass ? Div.dataset.animatioendClass : 'AntSuperset_PopUp_Dongs'} ${tims ? tims : .5}s`;
