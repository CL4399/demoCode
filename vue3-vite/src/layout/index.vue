<template>
    <Layout id="layout-content">
        <LayoutHeader class="header" style="display: flex; align-items: center; justify-content: center" :style="{ backgroundColor: storeObj.primaryColor, color: storeObj.textColor }">
            ...
            >
                <MenuItem :key="(item.id as string)" @click="chooseMenu(item)" v-for="item of routerInfo">{{ item.name }} </MenuItem>
            </Menu>
            ...
            <Layout style="height: 100%">
                <LayoutSider width="200" :collapsed="collapsed" :style="{ backgroundColor: storeObj.primaryColor, color: storeObj.textColor }">
                    <Menu :style="{ backgroundColor: storeObj.primaryColor }" v-model:selectedKeys="selectedKeys2" v-model:openKeys="openKeys" mode="inline">
                        <SubMenu :style="{ backgroundColor: storeObj.primaryColor, color: storeObj.textColor }" :key="(item.id as string)" v-for="item of navItemInfo" @click="chooseSubMenu(item)">
                            <template #title>
                                <span>
        ...
        <LayoutFooter style="text-align: center" :style="{ backgroundColor: storeObj.primaryColor, color: storeObj.textColor }"> Ant Design ©2018 Created by Ant UED </LayoutFooter>
    </Layout>
</template>
<script lang="ts">
